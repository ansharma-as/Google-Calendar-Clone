...
const Sidebar = ({ onCreateEvent, currentDate, onDateChange, isCollapsed }) => {
  ...
  const renderMiniCalendar = () => {
    ...
    return (
      <div className={`rounded-lg p-2 ${isDark ? 'bg-neutral-900' : 'bg-gray-50'}`}>
        {/* Mini Calendar Header */}
        <div className="flex items-center justify-between mb-3">
          <h3 className={`text-sm font-medium ${isDark ? 'text-neutral-100' : 'text-gray-900'}`}>
            {format(miniCalendarDate, 'MMMM yyyy')}
          </h3>
          <div className="flex items-center space-x-1">
            <button
              onClick={handlePrevMonth}
              className={`p-1 rounded transition-colors ${isDark ? 'hover:bg-neutral-700' : 'hover:bg-gray-100'}`}
            >
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${isCurrentDay && !isSelected ? 'bg-blue-600 text-white font-semibold' : ''}
  ...
  if (isCollapsed) {
    return (
      <aside className={`absolute top-20 left-6 w-14 h-14 rounded-xl flex flex-col items-center shadow-2xl shadow-neutral-500 ${isDark ? 'bg-neutral-800 border-neutral-700' : 'bg-white border-gray-200'}`}>
        <button
          onClick={onCreateEvent}
          className={`absolute rounded-xl w-full h-full flex items-center justify-center shadow-md transition-all hover:shadow-lg ${isDark ? 'bg-neutral-800 hover:bg-neutral-700' : 'bg-white hover:bg-gray-50'}`}
        >
          <Plus className={`w-6 h-6 ${isDark ? 'text-white' : 'text-gray-600'}`} />
        </button>
      </aside>
    );
  }
  ...
  return (
    <aside className={`w-64 p-4 flex flex-col ${isDark ? 'bg-neutral-900 border-neutral-700' : 'bg-gray-50 border-gray-200'}`}>
      {/* Create Button */}
      <button
        onClick={onCreateEvent}
        className={`flex w-36 text-sm items-center mb-6 px-3 py-1 border rounded-3xl shadow-sm transition-all hover:shadow-md ${isDark ? 'bg-neutral-800 hover:bg-neutral-700 border-neutral-700' : 'bg-white hover:bg-gray-50 border-gray-300'}`}
      >
        <div className={`w-12 h-12 flex items-center justify-center ${isDark ? ' border-neutral-700' : ' border-white'}`}>
          <Plus className={`w-5 h-5 ${isDark ? 'text-white' : 'text-gray-600'}`} />
        </div>
      ...
      <div className="mt-6">
        ...
        <div className="space-y-2">
          <label className={`flex items-center space-x-2 cursor-pointer px-2 py-1 rounded ${isDark ? 'hover:bg-neutral-800' : 'hover:bg-gray-50'}`}>
            <input
              type="checkbox"
              defaultChecked
